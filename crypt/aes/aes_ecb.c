...
void aes_key_en_schedule(const unsigned char *key, 
...
void aes_key_de_schedule(const unsigned char *key, 
                           unsigned int keybits, 
                           unsigned int *RK,
                           unsigned char* NR) 
{
    ...
    aes_key_en_schedule(key, keybits, RK1, NR);
...
void aes_encrypt(const unsigned int *RK, 
...
void aes_decrypt(const unsigned int *RK, 
                    const unsigned char* NR,
                    const unsigned char *input, 
                    unsigned char *output)
{
...
void aes_crypt_ecb(unsigned char mode,
                    ...
                    unsigned char *output)
{
    if (mode == ENCRYPT)
    {
        aes_encrypt(RK, NR, (const unsigned char*)input, output);
    }
    else
    {
        aes_decrypt(RK, NR, (const unsigned char*)input, output);
    }
}

void aes_crypt_cbc(unsigned char mode,
                    unsigned int length,
                    unsigned char iv[16],
                    const unsigned int *RK, 
                    const unsigned char* NR,
...
{
    unsigned int roundkeys[68] = {0};
    unsigned char i = 0;
    unsigned char numberOfRound = 0;
    ...
    // key schedule
    aes_key_en_schedule(Key, KeyLen * 8, roundkeys, &numberOfRound);
    
    for (i = 0; i < BlockCount; i++)
    {
        aes_crypt_ecb(ENCRYPT, roundkeys, &numberOfRound, &PlainContent[i * AES_BLOCK_SIZE], &CipherContent[i * AES_BLOCK_SIZE]);
    }
    return 0;
}
...
int aes_decrypt_ecb(const unsigned char* Key,const unsigned char KeyLen,unsigned char* CipherContent,unsigned char* PlainContent,const int BlockCount)
{
    ...
    // key schedule
    aes_key_de_schedule(Key, KeyLen * 8, roundkeys, &numberOfRound);
    
    for (i = 0; i < BlockCount; i++)
    {
        aes_crypt_ecb(DECRYPT, roundkeys, &numberOfRound, &CipherContent[i * AES_BLOCK_SIZE], &PlainContent[i * AES_BLOCK_SIZE]);
    }
    return 0;
}
...
void main(void)
{
  ...
  memcpy((void*)plain,data,32);
  aes_encrypt_ecb(key,16,plain,cipher,2);
  printf("sc_aes_ecb cipher data: \n");
  ...
  printf(("\n"));   
  if(aes_decrypt_ecb(key,16,cipher,plain,2) == 0x0)
  {
      printf("sc_aes_ecb plain data: \n");
      for(i = 0; i < 32; i++)
      {
         printf("%02x ",plain[i]);
      }
